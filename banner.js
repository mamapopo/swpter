/**
 * Created by userChen on 2017/3/17.
 */
//轮播图组件
function Banner(option){ //option 组件配置
    var container = document.createDocumentFragment(), //创建轮播图容器元素
        ...
        autoTimer; //设置定时器
        for(var i=0,l=arrImage.length;i<l;i++){
    		arrIndicator[i].classList.add("button");
    	}
    ...
    arrIndicator[0].classList.add("bcurrent"); //为第一个指针添加当前被选中类名
    start(); //执行启动轮播
    //创建单个图片元素方法
    ...
    function createController(className, type){ //className 翻页元素类名, type (false:前翻页, true:后翻页)
        ...
        polyline.setAttribute("stroke-width", 5); //设置描边宽度
    ...
    function setIndex(index){ //index 根据指针下标设置为当前下标
        ...
        arrIndicator[currentIndex].classList.remove("bcurrent"); //为轮播图原当前下标对应指针移除当前被选中类名
        ...
        arrIndicator[currentIndex].classList.add("bcurrent"); //为轮播图当前下标对应指针添加当前被选中类名
    }
    //启动轮播方法
    function start(){
